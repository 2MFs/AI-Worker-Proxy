...
- Node.js 26+ installed (see `.nvmrc`)
- Cloudflare account
...
Edit `routes.json` in the repo root. It is bundled into the Worker script, so it can be as
large as you like — the old 5.1 kB limit applied to the `ROUTES_CONFIG` text binding only.
...
If you deploy through GitHub Actions, you normally do not edit this file: the workflow
overwrites it with the `ROUTES_CONFIG` repository variable. A variable holds at most 48 kB,
so a larger config is pasted in parts — `ROUTES_CONFIG`, `ROUTES_CONFIG_2` ... `ROUTES_CONFIG_9`
are concatenated verbatim, in order, before being written to `routes.json`. See
[PRIVATE_CONFIG.md](PRIVATE_CONFIG.md).
...
### "Text binding 'ROUTES_CONFIG' is too large"

Old setups injected the routes as a text binding, which Cloudflare caps at 5.1 kB. The routes
now travel in `routes.json` inside the script — make sure no `[vars] ROUTES_CONFIG` is left in
your `wrangler.toml`, it would shadow the bundled file.
